...
CREATE TABLE IF NOT EXISTS bronze.castaway_details (
    ...
    CONSTRAINT ck_castaway_details_id CHECK (castaway_id IS NOT NULL AND castaway_id <> '')
);
...
-- ============================================================================
-- Gold Layer: Feature-ready tables for survivor winner prediction
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS gold.castaway_episode_features (
    ...
    label_finalist BOOLEAN,
    label_place INT,
    generated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_gold_castaway_episode_features UNIQUE (castaway_key, episode_key),
    CONSTRAINT fk_gold_castaway FOREIGN KEY (castaway_key) REFERENCES silver.dim_castaway (castaway_key),
    CONSTRAINT fk_gold_season FOREIGN KEY (season_key) REFERENCES silver.dim_season (season_key),
    CONSTRAINT fk_gold_episode FOREIGN KEY (episode_key) REFERENCES silver.dim_episode (episode_key)
);

CREATE TABLE IF NOT EXISTS gold.season_prediction_features (
    season_feature_key BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    season_key BIGINT NOT NULL,
    version_season TEXT NOT NULL,
    feature_snapshot_episode INT,
    features JSONB NOT NULL,
    label_winner_castaway_id TEXT,
    generated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT uq_gold_season_prediction_features UNIQUE (season_key, feature_snapshot_episode),
    CONSTRAINT fk_gold_season_prediction_season FOREIGN KEY (season_key) REFERENCES silver.dim_season (season_key),
    CONSTRAINT fk_gold_season_prediction_winner FOREIGN KEY (label_winner_castaway_id) REFERENCES bronze.castaway_details (castaway_id)
);
