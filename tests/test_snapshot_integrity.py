...
def test_voted_for_winner_is_a_vote():
    ...
    assert (ja["voted_for_winner"] <= (ja["voted_for_finalist_name"].astype(str) == "1.0")).all()
    assert (ja["voted_against_winner"] <= (ja["voted_for_finalist_name"].astype(str) == "1.0")).all()
    ...
    ballots = load_table("jury_votes", layer="bronze")
    cast = ballots[ballots["vote"].astype(str) == "1.0"].groupby(["castaway_id", "version_season"]).size()
    per_juror = ja.groupby(["castaway_id", "version_season"])[["voted_for_winner", "voted_against_winner"]].sum().sum(axis=1)
    assert per_juror.equals(cast.reindex(per_juror.index).fillna(0).astype(per_juror.dtype))
