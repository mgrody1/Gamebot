...
GOLD = {
    ...
    "is_african": COMMON["african"],
    "is_asian": COMMON["asian"],
    "is_latin_american": COMMON["latin_american"],
    "is_native_american": COMMON["native_american"],
    "is_bipoc": COMMON["bipoc"],
    "is_lgbt": COMMON["lgbt"],
    "season_number": "Season number within the version",
...
def main():
    ...
    missing = [
        ...
        for c in cs
        if "(no description yet)" in describe(t, c)
    ]
    print(
        f"wrote {total} column descriptions into dbt/models/silver/schema.yml and gold/schema.yml; {len(missing)} without a hand-written line: {missing[:10]}"
    )
