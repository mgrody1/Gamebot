...
castaway_base AS (
    ...
    JOIN {{ ref('dim_castaway') }} dc ON dc.castaway_key = bcs.castaway_key
    JOIN {{ ref('dim_season') }} ds ON ds.season_key = bcs.season_key
),
castaway_age AS (
    SELECT
        castaway_id,
...
challenge_events AS (
    SELECT
        ...
        fcr.sit_out
    FROM {{ ref('fact_challenge_results') }} fcr
    JOIN {{ ref('dim_challenge') }} dch ON dch.challenge_key = fcr.challenge_key
),
challenge_totals AS (
    SELECT
        castaway_key,
...
vote_events AS (
    SELECT
        ...
        fvh.tribe_status
    FROM {{ ref('fact_vote_history') }} fvh
),
vote_totals AS (
    SELECT
        castaway_key,
...
votes_received AS (
    ...
    FROM vote_events ve
    JOIN {{ ref('dim_castaway') }} dc ON dc.castaway_id = ve.target_castaway_id
...
tribe_composition AS (
    ...
    FROM {{ ref('fact_tribe_membership') }} tm
    JOIN {{ ref('dim_castaway') }} dc ON dc.castaway_key = tm.castaway_key
...
jury_original_tribe AS (
    ...
    FROM {{ ref('fact_jury_votes') }} jv
    JOIN {{ ref('bridge_castaway_season') }} finalist ON finalist.castaway_key = jv.finalist_castaway_key AND finalist.season_key = jv.season_key
    JOIN {{ ref('bridge_castaway_season') }} juror_bc ON juror_bc.castaway_key = jv.juror_castaway_key AND juror_bc.season_key = jv.season_key
